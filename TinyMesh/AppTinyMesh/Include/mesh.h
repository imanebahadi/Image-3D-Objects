...
public:
  ...
  explicit Mesh(const Disque& disque);
  explicit Mesh(const Sphere& sphere);
  explicit Mesh(const Tetraedre& tetraedre);
  explicit Mesh(const tore& Tore);
  explicit Mesh(const capsule& capsule);
  explicit Mesh(const Cone& cone);
  explicit Mesh(const Pyramide& pyra);
  explicit Mesh(const Cylindre& cylindre);
